...
# Per-IP rate limiting for booking endpoint (persists across warm Lambda invocations)
BOOKING_RATE_LIMIT = 5          # max bookings per IP per window
BOOKING_RATE_WINDOW = 3600      # 1 hour window in seconds
_booking_requests = defaultdict(list)  # ip -> [timestamps]


def check_booking_rate_limit(source_ip):
    """Return True if the request is within rate limits, False if exceeded."""
    now = time.time()
    cutoff = now - BOOKING_RATE_WINDOW
    # Prune old entries
    _booking_requests[source_ip] = [t for t in _booking_requests[source_ip] if t > cutoff]
    if len(_booking_requests[source_ip]) >= BOOKING_RATE_LIMIT:
        return False
    _booking_requests[source_ip].append(now)
    return True
...
def get_available_slots(date_str):
    """Get available 30-min slots for a given date (Eastern Time)"""
    ...
    # Query Google Calendar — use RFC3339 with offset for Eastern Time
    # April-November is EDT (UTC-4), November-March is EST (UTC-5)
    # Using timeZone param lets Google handle the conversion
    events_result = service.events().list(
        calendarId=CALENDAR_ID,
        timeMin=start_of_day.isoformat() + '-04:00',
        timeMax=end_of_day.isoformat() + '-04:00',
        singleEvents=True,
        orderBy='startTime'
    ).execute()
    ...
    for event in events_result.get('items', []):
        ...
        # Parse and strip timezone info for naive comparison in Eastern
        start_dt = datetime.fromisoformat(start)
        end_dt = datetime.fromisoformat(end)
        # Strip tzinfo for comparison with naive Eastern times
        busy_times.append((
...
def send_confirmation_email(name, email, date_str, time_str, meet_link=''):
    ...
    # HTML-escape user-supplied values for safe embedding in HTML
    safe_first_name = html_escape(first_name)
    safe_name = html_escape(name)

    # Validate meet_link is a safe URL
    if meet_link and not meet_link.startswith('https://'):
        meet_link = ''
...
<body style="margin:0; padding:0; background:#f4f4f5;
    ...
    <table width="560" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="padding:24px 40px 0; text-align:center;">
            <h2 style="margin:0 0 8px; font-size:24px; font-weight:700; color:#111;">
                You're all set, {safe_first_name}!
            </h2>
    ...
    print(f"Confirmation emails sent for booking")
    return True
...
def book_appointment(date_str, time_str, name, email, phone):
    ...
    Checks for conflicts before booking — rejects double-bookings."""
    service = get_calendar_service()
    ...
    if time_str not in available_slots:
        raise ValueError(f"Time slot {time_str} on {date_str} is not available. Available slots: {', '.join(available_slots) if available_slots else 'none'}")
    ...
    try:
        ...
        print(f"Failed to send confirmation email: status={e.response.status_code}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'POST' and 'book' in path:
            ...
            if source_ip and not check_booking_rate_limit(source_ip):
                return {
                    'statusCode': 429,
                    'headers': headers,
                    'body': json.dumps({'error': 'Too many booking requests. Please try again later.'})
                }
