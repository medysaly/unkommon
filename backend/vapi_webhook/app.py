...
def verify_vapi_signature(event):
    ...
    if not VAPI_WEBHOOK_SECRET:
        print("SECURITY: VAPI_WEBHOOK_SECRET not configured. Rejecting request.")
        return False
...
def handle_book_appointment(args):
    ...
    try:
        ...
        # Handle conflict (slot already taken)
        if resp.status_code == 409:
            return f"That time slot is not available. {data.get('error', 'Please check availability and pick a different time.')}"
        ...
        return "Unable to book the appointment right now. Please try again later."
...
def handle_tool_calls(body):
    ...
    for tc in tool_calls:
        ...
        print(f"Tool call: {fn_name} completed")
        results.append({
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"Vapi webhook received: type={message_type}")
